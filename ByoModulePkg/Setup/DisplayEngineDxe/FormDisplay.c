    ...
    *HighlightMenu = FindHighLightMenuOption(NULL);
    ...
    if (IsSavedHighlightStatement (FormData->HighLightedStatement)) {
      ...
      *HighlightMenu = FindHighLightMenuOption(NULL);
      ...
      //
      // Update TopOfScreen when the gMenuOption may be changed.
      //
      Length = BottomRow - TopRow + 1;
      HighlightPos = *HighlightMenu;
      TopPos = FindTopOfScreenMenuOption();

      Distance = 0;
      Link = TopPos;
      while (Link != &gMenuOption) {
      ...
      if (Distance > Length) {
        ...
        while (Link != &gMenuOption) {
          ...
          if (Length < Distance + MenuOption->Skip) {
            TopPos = Link->ForwardLink;
            break;
          }
  ...
  if (MenuOption->Description == NULL || MenuOption->Description[0] == '\0') {
    ...
    for (Index = 0; GetLineByWidth (MenuOption->Description, PromptWidth, &GlyphWidth, &Index, &OutputString) != 0x0000;) {      
      if ((Temp == 0) && (Row <= BottomRow)) { 
        ...
        if (Statement->OpCode->OpCode == EFI_IFR_REF_OP && MenuOption->Col >= 2 && MenuOption->Row == Row) {
          //
  ...
  )
{
  ...
  for(Index = 0; Index < HandleCount; Index ++){
    ...
    switch(Type){
      case SetupHookF4:
        if (NULL != ptSetupSaveNotify->HookF4) { 		  	
          Status = ptSetupSaveNotify->HookF4 (ptSetupSaveNotify);
        }
        break;
      default:
        break;
      ...
      if (Repaint) {
        ...
        while (Row <= BottomRow) {
          if ((FormData->Attribute & HII_DISPLAY_MODAL) != 0) {
            ...
            PrintStringAtWithWidth(gStatementDimensions.LeftColumn, Row++, L"", gStatementDimensions.RightColumn - gHelpBlockWidth - gStatementDimensions.LeftColumn - 1);
          }
        }
      ...
      case CHAR_NULL:
        ...
        if (((FormData->Attribute & HII_DISPLAY_MODAL) != 0) && (Key.ScanCode == SCAN_ESC || Index == mScanCodeNumber)) {
          ...
          if (Key.ScanCode == SCAN_F4 && 
              CompareGuid(&FormData->FormSetGuid, &gTrustedComputingSetupConfigGuid)) {
            TriggerSetupNotify (SetupHookF4);
            Repaint = TRUE;
            NewLine = TRUE;
            ControlFlag = CfRepaint;
          }
  ...
  )
{
  ...
  //
  // Check gOptionBlockWidth for ScrollBar.
  //	
  TotalSkipLines = 0;
  for (Link = gMenuOption.ForwardLink; Link != &gMenuOption; Link = Link->ForwardLink) {
    TmpMenuOption = MENU_OPTION_FROM_LINK (Link);
  ...
  if ((gStatementDimensions.BottomRow - gStatementDimensions.TopRow + 1) > TotalSkipLines) {
    gOptionBlockWidth += 1;
  }	
