  ...
  )
{
  ...
  if ((gLibIsFirstForm) || ((FormData->Attribute & HII_DISPLAY_MODAL) != 0)) {
    ...
    //
    GopBltArea(0, gScreenDimensions.RightColumn, 2, gScreenDimensions.BottomRow-3, EFI_BACKGROUND_LIGHTGRAY);
  }
  ...
  )
{
  ...
  for (Index = Top; Index + 2 < Bottom; Index++, Count++) {
    ...
    if (String[0] == CHAR_NULL) {
      ...
      //
      GopBltArea (Start, End, Index + 1, Index + 1, GetPopupColor ());
    } else if (String[0] == L' ') {
      //
      // Passing in a space results in the assumption that this is where typing will occur
      //
      GopBltArea (Start + 1, End - 1, Index + 1, Index + 1, POPUP_INVERSE_TEXT | POPUP_INVERSE_BACKGROUND);
      PrintStringAt (
  ...
  )
{
  ...
  //
  do {
    CreateDialog (&Key, gLibEmptyString, FormTitle, gAreYouSure, gLibEmptyString, NULL);
  } while
  (
  ...
  )
{
  ...
  EFI_GRAPHICS_OUTPUT_BLT_PIXEL    BackGroundBlt_Lightgray = {0x98, 0x98, 0x98, 0x00};  // LIGHTGRAY
  ...
  DEBUG((EFI_D_ERROR, " GopBltArea(), Locate GOP :%r.\n", Status));
  if (!EFI_ERROR(Status)) {
  ...
  DEBUG((EFI_D_ERROR, " GopBltArea(), GOP BLT :%r.\n", Status));
  }
...
{
  InitializePlatformLanguage(TRUE);
  ...
  )
{
  ...
  //
  gST->ConOut->EnableCursor (gST->ConOut, FALSE);
  GopBltArea (StartColum, StartColum + 46, StartRow, StartRow + 10, EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE);
