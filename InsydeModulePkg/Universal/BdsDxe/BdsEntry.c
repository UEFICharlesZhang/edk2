...
/**
  Initialize BDS_COMMON_OPTION instance by input H2O_BDS_LOAD_OPTION instance.

  @param[in]  BootOption        A pointer to H2O_BDS_LOAD_OPTION instance.
  @param[in]  CommonBootOption  A pointer to BDS_COMMON_OPTION instance.

  @retval EFI_SUCCESS           Initialize BDS_COMMON_OPTION instance successfully.
  @retval EFI_INVALID_PARAMETER BootOption is NULL or CommonBootOption is NULL.
**/
STATIC
EFI_STATUS
InitCommonBootOption (
  IN   H2O_BDS_LOAD_OPTION    *BootOption,
  OUT  BDS_COMMON_OPTION      *CommonBootOption
  )
{
  if (BootOption == NULL || CommonBootOption == NULL) {
    return EFI_INVALID_PARAMETER;
  }
  ZeroMem (CommonBootOption, sizeof (BDS_COMMON_OPTION));
  CommonBootOption->Signature       = BDS_LOAD_OPTION_SIGNATURE;
  CommonBootOption->DevicePath      = BootOption->DevicePath;
  CommonBootOption->OptionName      = BootOption->LoadOrderVarName;
  CommonBootOption->OptionNumber    = (UINTN) BootOption->LoadOrder;
  CommonBootOption->Attribute       = BootOption->Attributes;
  CommonBootOption->Description     = BootOption->Description;
  CommonBootOption->LoadOptions     = BootOption->LoadOptionalData;
  CommonBootOption->LoadOptionsSize = BootOption->LoadOptionalDataSize;
  CommonBootOption->StatusString    = BootOption->StatusString;
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  BDS_COMMON_OPTION             CommonBootOption;
  ...
  if (gBdsServices->BootNextLoadOption) {
    //
    ...
    if (Status != EFI_SUCCESS) {
      ...
      if (!EFI_ERROR (SystemConfigStatus) &&
        ...
        if (!(((DevicePathType (BootOption->DevicePath) == MESSAGING_DEVICE_PATH) &&
          ...
          InitCommonBootOption (BootOption, &CommonBootOption);
          PlatformBdsBootFail (&CommonBootOption, Status, ExitData, ExitDataSize);
        }
      }
      ...
      //
      InitCommonBootOption (BootOption, &CommonBootOption);
      PlatformBdsBootSuccess (&CommonBootOption);
  ...
  )
{
  ...
  //
  // Lock other sensitive variables
  //
