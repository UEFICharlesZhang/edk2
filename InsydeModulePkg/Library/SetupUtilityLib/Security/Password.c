...
STATIC BOOLEAN                     mSetHddPassword       = FALSE;
STATIC BOOLEAN                     mHDDPswdBootIn        = FALSE;
STATIC BOOLEAN                     mUnLockHddPassword    = FALSE;
STATIC BOOLEAN                     mSetAllHddPassword    = FALSE;
STATIC UINT16                      mSetAllCurrentPassword[HDD_PASSWORD_MAX_NUMBER + 1];
STATIC BOOLEAN                     mDisableAllHddFlag  = FALSE;
STATIC UINT16                      mSaveItemMapping[0xf][2];
  ...
  )
{
  ...
  if (mSetHddPassword == TRUE && HddPasswordDataInfo[CurrentHddIndex].Flag != SECURITY_LOCK) {
    ...
    if (HddPasswordDataInfo[CurrentHddIndex].Flag == CHANGE_PASSWORD) {
      //
      ...
      if (StrCmp (mSetAllCurrentPassword, Password) == 0) {
        ...
        //
        return EFI_SUCCESS;
      } else if (MasterOrUser == USER_PSW && HddPasswordDataInfo[CurrentHddIndex].InputString[0] != 0) {
        if (StrCmp (HddPasswordDataInfo[CurrentHddIndex].InputString, Password) == 0) {
          //
      ...
      } else if (MasterOrUser == MASTER_PSW && HddPasswordDataInfo[CurrentHddIndex].MasterInputString[0] != 0) {
        if (StrCmp (HddPasswordDataInfo[CurrentHddIndex].MasterInputString, Password) == 0) {
          //
  ...
  )
{
  ...
  for (Index = 0; Index < SUCInfo->NumOfHarddiskPswd; Index++) {
    ...
    if (SUCInfo->HarddiskPassword[Index].MasterFlag == CHANGE_PASSWORD ||
        (SUCInfo->HarddiskPassword[Index].Flag == CHANGE_PASSWORD &&
         SUCInfo->HarddiskPassword[Index].MasterFlag == MASTER_PASSWORD_GRAYOUT &&
         SUCInfo->HarddiskPassword[Index].MasterInputString[0] != 0) ) {
      //
      // Change Password
...
/**
 Update HDD security status information.

 @param [in]   HiiHandle
 @param [in]   IdentifyData
 @param [in]   HddPortNumber
 @param [in]   Device           '0' is master, '1' is slave in IDE mode
 @param [in]   TokenToUpdate    EFI-String token
 @param [in]   CallBackFormId   The Ide device form ID number
 @param [in]   LanguageString

 @retval EFI_UNSUPPORTED        Could not found HDD
 @retval EFI_SUCCESS            Update form successfully
  ...
  )
{
  ...
  STRING_REF                            Token;
  CHAR16                                *StringPtr;
  HDD_PASSWORD_INFO_DATA                *HddPasswordScuData;
  HDD_SECURITY_INFO_DATA                *HddInfo;
  UINTN                                 NumOfHdd;
  SETUP_UTILITY_CONFIGURATION           *SUCInfo;
  SETUP_UTILITY_BROWSER_DATA            *SuBrowser;
  ...
  HddPasswordScuData = SUCInfo->HarddiskPassword;
  NumOfHdd = SUCInfo->NumOfHarddiskPswd;

  if ((NumOfHdd == 0) || (HddPasswordScuData == NULL)) {
   return EFI_UNSUPPORTED;
  }
  ...
  if ((IdentifyData->config & bit(15)) == 0) {
    //
    // Only support for ATA device
    //

    for (Index = 0; Index < NumOfHdd; Index++) {
      //
      // Search match HDD info
      //
      HddInfo = HddPasswordScuData[Index].HddDataInfo;
      if (HddInfo->ControllerMode == ATA_IDE_MODE) {
        if (HddInfo->MappedPort != HddPortNumber) {
          continue;
        }
      } else {
        if (HddInfo->PortNumber != HddPortNumber) {
          continue;
        }
      }
      ...
      // Get string token
      //
      if (HddPasswordScuData[Index].Flag == SECURITY_LOCK) {
        //
        // HDD Security Enable [Lock]
        //
        Token = STRING_TOKEN (STR_HDD_PSW_LOCK);
      } else if (HddPasswordScuData[Index].Flag == CHANGE_PASSWORD) {
        //
        // HDD Security Enable [Change]
        //
        Token = STRING_TOKEN (STR_HDD_PSW_CHANGE);
      } else if (HddPasswordScuData[Index].Flag == DISABLE_PASSWORD) {
        //
        // HDD Security Enable [Disable]
        //
        Token = STRING_TOKEN (STR_HDD_PSW_DISABLE);
      } else if (HddPasswordScuData[Index].Flag == ENABLE_PASSWORD) {
        //
        // HDD Security Unlock
        //
        Token = STRING_TOKEN (STR_HDD_PSW_UNLOCK);
        ...
        //
        //HDD don't access password
        //
        Token = STRING_TOKEN (STR_HDD_PSW_NONACCESS);
      }

      HddPasswordScuData[Index].LabelIndex = CallBackFormId;
      break;
    }
  }
  ...
  if (Token != 0) {
    StringPtr = HiiGetString (
                  SUCInfo->MapTable[SecurityHiiHandle].HiiHandle,
                  Token,
                  NULL
                  );
    HiiSetString (
...
/**
 ...
 @param [out]  HaveSecurityData   TRUE: Have security data
                                  FALSE: No security data.
 @param [out]  HarddiskInfoIndex  If there is the matched security data, this value is the index in SECURITY_INFO_DATA array.

 @retval EFI_SUCCESS            Check have security data successfully
  ...
  )
{
  ...
  NumOfHdd = SCUInfo->NumOfHarddiskPswd;
  HddPasswordScuData = SCUInfo->HarddiskPassword;
