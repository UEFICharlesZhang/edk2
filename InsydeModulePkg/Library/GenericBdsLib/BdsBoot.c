  ...
  for (Index = 0; Index < NumberFileSystemHandles; Index++) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if (!BlkIo->Media->RemovableMedia &&
        ...
        //
        if (FeaturePcdGet (PcdH2OBdsDefaultBootListGenericOsSupported) && IsGenericUefiBootOs (FileSystemHandles[Index])) {
    ...
    if (!HaveDefaultRemovableFile (FileSystemHandles[Index])) {
      ...
      case BDS_EFI_SDHC_BOOT:
        if (BlkIo->Media->RemovableMedia) {
          if (SdMmcNumber == 0) {
            UnicodeSPrint (Buffer, sizeof (Buffer), L"%s", BdsLibGetStringById (STRING_TOKEN (STR_DESCRIPTION_SDMMC)));
          } else {
            UnicodeSPrint (Buffer, sizeof (Buffer), L"%s %d", BdsLibGetStringById (STRING_TOKEN (STR_DESCRIPTION_SDMMC)), SdMmcNumber);
          }
          SdMmcNumber++;
        } else {
          if (EmmcNumber == 0) {
