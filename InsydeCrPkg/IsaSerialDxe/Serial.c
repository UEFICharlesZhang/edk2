...
EFI_EVENT                                    mCrOpromInstallNotifyEvent;
VOID                                         *mCrOpromInstallRegistration;
UINT8                                        mSwFlowControl_Transfer = XON;
...
{ 
  ...
  mSwFlowControl_Transfer = SwFcState;
  if (mCrOpROMInfoProtocol->CrOpROMInstalled) {
    CrOpromInfo = NULL;
    CrOpromInfo = mCrOpROMInfoProtocol->CrGetCommonArea (mCrOpROMInfoProtocol);
    if (CrOpromInfo != NULL) {
      CrOpromInfo->SwFlowControlState = SwFcState;
    }
  }
...
{ 
  ...
  SwFcState = mSwFlowControl_Transfer;
  ...
  if (mCrOpROMInfoProtocol->CrOpROMInstalled) {
    ...
    CrOpromInfo = mCrOpROMInfoProtocol->CrGetCommonArea (mCrOpROMInfoProtocol);
    if (CrOpromInfo != NULL) {
      SwFcState = CrOpromInfo->SwFlowControlState;
    }
  }
  ...
  )
{
  ...
  CrOpromInfo->SwFlowControlState = mSwFlowControl_Transfer;
...
{
  if (SerialDevice->CrFlowControl == UART_FLOW_CONTROL_XON_XOFF && GetSwFcState() == XON) { 
    WRITE_THR (SerialDevice->IsaIo, SerialDevice->BaseAddress, FcData);
  }
}
    ...
    if (!IsDevicePathEnd (RemainingDevicePath)) {
      ...
      if (IsUartFlowControlNode (FlowControlNode)) {
        ...
        if (((ReadUnaligned32 (&FlowControlNode->FlowControlMap) & ~UART_FLOW_CONTROL_HARDWARE) != 0) &&
            ((ReadUnaligned32 (&FlowControlNode->FlowControlMap) & ~UART_FLOW_CONTROL_XON_XOFF) != 0)) {
          goto Error;
        }
      }
    }
...

{
  ...
  if (FlowControlMap == UART_FLOW_CONTROL_XON_XOFF) {
    SerialDevice->CrFlowControl = UART_FLOW_CONTROL_XON_XOFF;
    Status = gBS->CreateEvent (
                    EVT_NOTIFY_SIGNAL,
                    TPL_NOTIFY,
                    CrOpromInstallNotifyFunction,
                    NULL,
                    &mCrOpromInstallNotifyEvent
                    );
    if( EFI_ERROR(Status)) {
      return Status;
    }
    ...
    do {
      ...
      if ((Lsr.Bits.Dr == 1) && !ReceiveFifoFull) {
        ...
        if (!ReceiveFifoFull) {
          ...
          Data = READ_RBR (SerialDevice->IsaIo, SerialDevice->BaseAddress);
          if (SerialDevice->CrFlowControl == UART_FLOW_CONTROL_XON_XOFF) {
            //
            // Igonore data when FC state as XOFF
            //
            if (Data == XON || Data == XOFF) {
              SetSwFcState (Data);
            } else if(GetSwFcState() == XON) {
              IsaSerialFifoAdd (&SerialDevice->Receive, Data);
            }
          } else {
            IsaSerialFifoAdd (&SerialDevice->Receive, Data);
  ...
  )
{
  ...
  SoftWareFlowControl (SerialDevice, XON);
  *Control      = 0;
  ...
  SoftWareFlowControl (SerialDevice, XOFF);
  gBS->RestoreTPL (Tpl);
