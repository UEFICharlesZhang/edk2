...
#include <Protocol/CrDeviceSupport.h>
...
EFI_CONSOLE_REDIRECTION_BDS_HOOK_PROTOCOL  mCrBdscr = {
  ...
  PlatformBDSConnectCRPnpTerminal
};
...
static UINT32 mCrFcBitMapTable [] = {
  0, 1, 16
};
  ...
  )
{
  ...
  CrDevCount = CrPolicy->CRSerialDevData.IsaDevCount + 
               ...
               CrPolicy->CRSerialDevData.NetDevCount;
  ...
  if (CRIsIsaSerialDevicePathInstance (DevicePath) || Type == ISA_SERIAL_DEVICE) {
  ...
  } else if (CRIsLanSerialDevicePathInstance (DevicePath) || Type == NET_SERIAL_DEVICE) {
    //
    // LAN serial
    //
    Start = CrPolicy->CRSerialDevData.IsaDevCount + 
    ...
    for (Index = Start; Index < CrDevCount; Index++) {
      if (CrPolicy->CRSerialDevData.CRDevice[Index].Type == NET_SERIAL_DEVICE) {
        *CrDeviceIndex = Index;
        return TRUE;
      }
    }
  ...
  )
{
  ...
  DevPath = TerminalDevicePath;
  do {
    DevicePath = (EFI_DEVICE_PATH_PROTOCOL *) CRLocateDevicePathNode (&DevPath, MESSAGING_DEVICE_PATH, MSG_VENDOR_DP);
    if ((DevicePath != NULL) && (DevicePath->Length[0] == (UINT8)sizeof (VENDOR_DEVICE_PATH))) {
      TerminalNode = (VENDOR_DEVICE_PATH*)DevicePath;
  ...
  )
{
  ...
  if (CrPolicy->CRFlowControl == CR_FLOW_CONTROL_HARDWARE || CrPolicy->CRFlowControl == CR_FLOW_CONTROL_SOFTWARE) {
    mFlowControlNode.FlowControlMap = mCrFcBitMapTable[CrPolicy->CRFlowControl];
...
ConnectTerminalDevice (
  IN EFI_CR_POLICY_PROTOCOL    *CrPolicy,
  IN CR_DEVICE_CATEGORY        CrDevCategory
  )
{
  EFI_STATUS                    Status;
  EFI_STATUS                    Result;
  CR_DEVICE_SUPPORT_PROTOCOL    *CrDevSupport;
  UINT16                        Index;
  UINTN                         HandleCount;
  EFI_HANDLE                    *Handles;
  ...
  if (CrDevCategory == CR_PnpDevice) {
    //
    // Connect Network serial device
    //
    Status = gBS->LocateHandleBuffer (
                      ByProtocol,
                      &gCrDeviceSupportProtocolGuid,
                      NULL,
                      &HandleCount,
                      &Handles
                      );
    ...
    if (!EFI_ERROR(Status)) {
      for (Index=0; Index < HandleCount; Index++) {
        ...
        Status = gBS->HandleProtocol (Handles[Index], &gCrDeviceSupportProtocolGuid, (VOID **)&CrDevSupport);
        if (EFI_ERROR(Status)) {
          continue;
        }

        if (CrDevSupport->CrDevCategory == CrDevCategory) {
          Status = CrDevSupport->ConnectTerminalDevice ();
          if (Status == EFI_SUCCESS)
            Result = EFI_SUCCESS;
        }
      }
  ...
  )
{
  ...
  if (TDManager->DeviceCount == 0) {
    Size = sizeof(EFI_CONSOLE_REDIRECTION_INFO) + (CR_MAX_SUPPORT_SERIAL_PORT * sizeof (EFI_CONSOLE_REDIRECTION_DEVICE));
    CRInfo = AllocateZeroPool (Size);
    CRInfo->Headless = Headless;
    CRInfo->DeviceCount = 0;
    return CRInfo;
  }
  ...
  for (Index = 0; Index < TDManager->DeviceCount; Index++) {
    ...
    CRInfo->CRDevice[Index].Type = Device->DeviceType;
    ...
    if (Device->DeviceType == ISA_SERIAL_DEVICE) {
    ...
    }	else if (Device->DeviceType == PCI_SERIAL_DEVICE) {
      //
      // PCI serial device
      //
	    DevPath = Device->DevicePath;
      ...
      //
      // Ignore USB and LAN serial device due to this type not supported in CROpROM
      //
      continue;
    }
  }
  ...
  )
{
  ...
  for (Index = 0; Index < TDManager->DeviceCount; Index++) {
    ...
    if ((TDManager->Device[Index].DeviceType == ISA_SERIAL_DEVICE) ||
        (TDManager->Device[Index].DeviceType == PCI_SERIAL_DEVICE)) {
  ...
  )
{
  ...
  for (DevCount = 0, Index = 0; Index < HandleCount; Index++) {
    ...
    //
    // Decide to add or not add the device
    //
    if (Status == EFI_SUCCESS) {
    ...
    if (DoAddDevice == TRUE) {
      ...
      } else if (CRIsUsbSerialDevicePathInstance (DevicePath)){
        DeviceType = USB_SERIAL_DEVICE;
      } else if (CRIsLanSerialDevicePathInstance (DevicePath)) {
        DeviceType = NET_SERIAL_DEVICE;
      } else {
        continue;
      }
  ...
  )
{
  ...
  if (CrPolicy->CREnable == CR_ENABLE) {
    Status = ConnectTerminalDevice (CrPolicy, CR_PnpDevice);
  }
  else {
  ...
  )
{
  ...
  if (CrPolicy->CREnable == CR_DISABLE) {
    return EFI_UNSUPPORTED;
  }

  //
  // Connect C.R. NonPnp terminal device
  //
  Status = ConnectTerminalDevice (CrPolicy, CR_NonPnpDevice);
  
  //
  // Connect C.R. Pnp terminal device
  //
  //PlatformBDSConnectCRPnpTerminal ();
