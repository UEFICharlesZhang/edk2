  ...
  )
{
  ...
  if (This->CrOpROMInstalled) {
    ...
    if (mCrEbdaMemManager != NULL) {
      ...
      //
      return (CR_OPROM_INFO *)((UINTN)mCrEbdaMemManager + (UINTN)mCrEbdaMemManager->CommonAreaOffset);
    }
  } else {
   if (mCrMemStart != 0) {
     CrEfiMemManager = (CR_EFI_MEM_MANAGER *)(UINTN)mCrMemStart;
     return (CR_OPROM_INFO *)(UINTN)(mCrMemStart + CrEfiMemManager->CommonAreaOffset);
   }
  }
  ...
  for (Index = 0; Index < gCrInfo->DeviceCount; Index++) {
    ...
    switch (gCrInfo->CRDevice[Index].Type) {
      ...
      case ISA_SERIAL_DEVICE:
        ...
        if (CrPolicy->CRFlowControl == CR_SERIAL_SW_FC) {
          CrOpromInfo = mCrOpROMInfo.CrGetCommonArea (&mCrOpROMInfo);
          if (CrOpromInfo != NULL) {
            if (CrOpromInfo->SwFlowControlState == XON) {
              if (EnableIntr) {
                u8 = XON;
              } else {
                u8 = XOFF;
              }
              gRootBridgeIo->Io.Write (gRootBridgeIo, EfiPciWidthUint8, IsaSerial->ComPortAddress + UART_THR_OFFSET, 1, &u8);
            }
          }
        }
        break;
  ...
  )
{
  ...
  for (DevCount = 0, Index = 0; Index < CRInfo->DeviceCount; Index++) {
    ...
    if (CRInfo->CRDevice[Index].Type == ISA_SERIAL_DEVICE) {
      ...
      //
      CRDevice[DevCount].Type = ISA_SERIAL_DEVICE;
      CRDevice[DevCount].Device.IsaDevice.PortAddress = CRInfo->CRDevice[Index].Device.IsaSerial.ComPortAddress;
      CRDevice[DevCount].Device.IsaDevice.Irq = CRInfo->CRDevice[Index].Device.IsaSerial.ComPortIrq;
      ...
      //
      CRDevice[DevCount].Type = PCI_SERIAL_DEVICE;
      CRDevice[DevCount].Device.PciDevice.Bus = CRInfo->CRDevice[Index].Device.PciSerial.Bus;
      CRDevice[DevCount].Device.PciDevice.DevFun = (CRInfo->CRDevice[Index].Device.PciSerial.Device << 3) | \
  ...
  )
{
  ...
  if (LocalRomImage != NULL || LocalRomSize != 0) {
    ...
    if ((CrPolicy->CROpROMLoadOnESegment & CR_OPROM_LOAD_ON_ESEG_FLAG) == CR_OPROM_LOAD_ON_ESEG_ENABLE) {
...
)
{
  ...
  // Load Console Redirection Option ROM
  //
  Status = LoadOpRomImage (CrPolicy);
  if (Status == EFI_SUCCESS) {
