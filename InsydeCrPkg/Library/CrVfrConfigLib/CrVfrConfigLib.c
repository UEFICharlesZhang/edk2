  ...
  )
{
  ...
  SerialDevCount = CRSerialDevInfo->IsaDevCount + CRSerialDevInfo->PciDevCount + CRSerialDevInfo->UsbDevCount;

  //
  // CrDeviceSupportProtocol be install when SolDxe driver dispatched
  //
  Status = gBS->LocateProtocol (&gCrDeviceSupportProtocolGuid, NULL, (VOID **)&CrDevSupport);

  if (!EFI_ERROR(Status)) {
    if (SerialDevCount >= CR_MAX_SUPPORT_SERIAL_PORT) return EFI_SUCCESS;
    CRSerialDevInfo->CRDevice[SerialDevCount].Type = NET_SERIAL_DEVICE;
    CRSerialDevInfo->NetDevCount = 1;
  }
  ...
  )
{
  ...
  // Get Usb device info
  //
  if (FeaturePcdGet(PcdH2OCRUsbSerialSupported)){
    GetUsbSerialInfo (CRSerialDevInfo, DoBusConnect);
  }

  GetSolInfo (CRSerialDevInfo, DoBusConnect);
  ...
  )
{
  ...
  //
  TotalDevNum = ReferenceDevInfo->IsaDevCount + ReferenceDevInfo->PciDevCount + 
                ReferenceDevInfo->UsbDevCount + ReferenceDevInfo->NetDevCount;
  for (Idx = 0; Idx < TotalDevNum; Idx++) {
    ...
    } else if (PlatformDev->Type == NET_SERIAL_DEVICE) {
      DevStart = ReferenceDevInfo->IsaDevCount + ReferenceDevInfo->PciDevCount + ReferenceDevInfo->UsbDevCount;
      DevNum   = ReferenceDevInfo->IsaDevCount + ReferenceDevInfo->PciDevCount + ReferenceDevInfo->UsbDevCount + ReferenceDevInfo->NetDevCount;
      for (CrDevIdx = DevStart; CrDevIdx < DevNum; CrDevIdx++) {
        CrDev = &ReferenceDevInfo->CRDevice[CrDevIdx];
        if (CrDev->Type == NET_SERIAL_DEVICE) {
          
          CopyMem (&UpdateDevInfo->CrSolConfig, &ReferenceDevInfo->CrSolConfig, sizeof (CR_SOL_CONFIG));
          break;
        }
      }
    }
  ...
  )
{
  ...
  //
  // Refresh for SOL infor
  //
  SolUpdateEntrys (&mCrIfrNvdata);
  ...
  )
{
  ...
  //
  // Load Cr SOL default config
  //
  SolConfigSetToDefault (&CRSerialDevInfo->CrSolConfig);
  ...
  )
{
  ...
  //
  // Init CR Scu Net Option
  //
  if (CRSerialDevInfo->NetDevCount > 0 && mCrSolScuSupport) {
    ...
    HiiCreateGotoOpCode (
      ...
      0,
      EFI_IFR_FLAG_CALLBACK,
      (UINT16)(CONSOLE_REDIRECTION_NET_FORM_ID)
      );
  }
  ...
  )
{
  ...
  //
  mCrSolScuSupport = TRUE;
  Status = SolConfigFormInit (HiiHandle, &PlatformCrDevInfo);
  if (Status != EFI_SUCCESS) {
    mCrSolScuSupport = FALSE;
  }
  ...
  )
{
  ...
  switch (QuestionId) {
    ...
    case KEY_SOL_ADMIN_MAC:  
      ...
      //
      // Update Sol device setting
      //
      SolConfigCallback (QuestionId, &mCrIfrNvdata);
    ...
    case KEY_CONSOLE_REDIRECTION_17 :
    case KEY_CONSOLE_REDIRECTION_18 :
      ...
      if (BrowserAccessSuccess) {        
        CopyMem (
          ...
          mCrIfrNvdata.CrSolConfig.Enable  = mCrConfigData.SolEnable;
          mCrIfrNvdata.CrSolConfig.UseDhcp = mCrConfigData.UseDhcp;
      }
