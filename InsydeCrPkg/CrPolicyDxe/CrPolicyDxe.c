...
EFI_CR_POLICY_PROTOCOL mCRPolicyTemplate = {
  ...
  CR_OPROM_LOAD_ON_ESEG_DISABLE, //CROpROMLoadOnESegment  
  {0},                           //CRSerialDevData
  {0},                           //CrDevDefaultAttr
  {0}                            //CrSolDefaultConfig,
};
  ...
  )
{
  ...
  // Init for SOL
  //
  ZeroMem (&CrPolicy->CrSolDefaultConfig, sizeof (CR_SOL_CONFIG));
  CrPolicy->CrSolDefaultConfig.Enable = TRUE;
  CrPolicy->CrSolDefaultConfig.UseDhcp = TRUE;
  CrPolicy->CrSolDefaultConfig.PortNumber = 23;
  StrCpy (CrPolicy->CrSolDefaultConfig.UserName, L"Insyde");
  StrCpy (CrPolicy->CrSolDefaultConfig.Password, L"123");  
