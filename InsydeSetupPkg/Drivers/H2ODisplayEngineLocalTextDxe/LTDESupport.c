  ...
  )
{
  ...
  do {
    ...
    GetPanelField (Panel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL, &ScreenField, &Panel->PanelField);
  ...
  )
{
  ...
  while (!IsNull (&mDEPrivate->Layout->PanelListHead, Link)) {
    ...
    if (Panel != NULL &&
        IsVisibility (Panel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL) &&
  ...
  GetPanelColorAttribute (TargetPanel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL, &PanelColorAttribute);
  ...
  )
{
  ...
  //
  DisplayString (CurrentPanelField->left, CurrentPanelField->bottom, BottomHorizonalLine);
  DisplayString (CurrentPanelField->left, CurrentPanelField->top   , TopHorizonalLine);

  Character[1] = CHAR_NULL;
  for (Index = 0; Index < VerticalLineHeight; Index++) {
    Character[0] = LeftVerticalLine[Index];
    DisplayString (CurrentPanelField->left , CurrentPanelField->top + 1 + Index, Character);

    Character[0] = RightVerticalLine[Index];
  ...
  )
{
  ...
  if (ConDevNode->GraphicsOut != NULL) {
    ScreenPanel = GetPanelInfoByType (&mDEPrivate->Layout->PanelListHead, H2O_PANEL_TYPE_SCREEN);
  ...
  )
{
  ...
  while (!IsNull (&mDEPrivate->Layout->PanelListHead, Link)) {
    ...
    if (CurrentPanel == NULL ||
        !IsVisibility (CurrentPanel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL) ||
        GetBorderWidth (CurrentPanel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL) == 0) {
      continue;
    }

    GetPanelColorAttribute (CurrentPanel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL, &PanelColorAttribute);
  ...
  )
{
  ...
  GetBackgroundColor (SetupMenuPanel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL, &SetupMenuPanelColor);
  BorderLineWidth = (INT32) GetBorderWidth (SetupMenuPanel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL);

  //
  // Set the position of each setup menu item which is relative to PanelRelField of parent panel
  //
  if ((SetupMenuPanel->OrderFlag & H2O_PANEL_ORDER_FLAG_HORIZONTAL_FIRST) == 0) {
    return EFI_UNSUPPORTED;
  }

  //
  // BUGBUG:
  // 1. Implement dynamically organize setup menu item when exceed one line (like hot key panel)
  // 2. Implement setup menu in vertical display
  // 3. If current setup menu is on second line, it should scroll to second line or display "..."
  //
  ...
  for (Index = 0; Index < SetupMenuControls->Count; Index++) {
    ...
    if ((Control->PageId >> 16) == (FBProtocol->CurrentP->PageId >> 16)) {
      mDEPrivate->PageTagSelected = Control;
    } else {
      BackupStr = NULL;
      if (!CurrentPageIsRoot) {
        BackupStr = Control->Text.String;
        Control->Text.String = CreateSpaceString (StringWidth);
      }
      DisplayNormalControls (1, Control);
      if (!CurrentPageIsRoot) {
        SafeFreePool ((VOID **) &Control->Text.String);
        Control->Text.String = BackupStr;
      }
    }
  }
  ...
  InflateRect (&CombineFields, -BorderLineWidth, -BorderLineWidth);
  CombineFields.right = CombineFields.left + StartX - 1;
  ClearField (SetupMenuPanelColor.Attribute, &CombineFields, NULL);

  if ((CombineFields.bottom - CombineFields.top) > 0) {
    CopyRect (&CombineFields, &SetupMenuField);
    InflateRect (&CombineFields, -BorderLineWidth, -BorderLineWidth);
    CombineFields.top++;
    ClearField (SetupMenuPanelColor.Attribute, &CombineFields, NULL);
  ...
  CopyRect (&CombineFields, &SetupMenuField);
  InflateRect (&CombineFields, -BorderLineWidth, -BorderLineWidth);
  CombineFields.left  = CombineFields.left + SetupMenuControls->ControlArray[SetupMenuControls->Count - 1].ControlField.right + 1;
  ClearField (SetupMenuPanelColor.Attribute, &CombineFields, NULL);
  ...
  )
{
  ...
  HotKeyPanel = GetPanelInfoByType (&mDEPrivate->Layout->PanelListHead, H2O_PANEL_TYPE_HOTKEY);
  if (HotKeyPanel == NULL) {
    return EFI_NOT_FOUND;
  }
  ...
  Status = GetPanelColorAttribute (HotKeyPanel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL, &PanelColorAttribute);
  if (EFI_ERROR (Status)) {
  ...
  //
  BorderLineWidth    = GetBorderWidth (HotKeyPanel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL);
  HotKeyPanelHeight  = (UINT32) (HotkeyField.bottom - HotkeyField.top + 1 - 2 * BorderLineWidth);
  HotKeyPanelWidth   = (UINT32) (HotkeyField.right - HotkeyField.left + 1 - 2 * BorderLineWidth);
  KeyCountPerLine    = HotKeyCount / HotKeyPanelHeight;
  KeyCountPerLine   += (HotKeyCount % HotKeyPanelHeight != 0) ? 1 : 0;
  ...
  for (Index = 0; Index < HotKeyCount; Index++) {
    ...
    SetRect (
      &EmptyField,
      X + (INT32) StrLen (HotKeyControls[Index].Text.String) + BorderLineWidth,
      HotkeyField.top + Y + BorderLineWidth,
      X + HotKeyControlWidth + BorderLineWidth,
      HotkeyField.top + Y + BorderLineWidth
      );
  ...
  if (BorderLineWidth != 0) {
    EmptyField.left   += BorderLineWidth;
    EmptyField.right  += BorderLineWidth;
    EmptyField.top    += BorderLineWidth;
    EmptyField.bottom -= BorderLineWidth;
  }
  ClearField (PanelColorAttribute, &EmptyField, NULL);
  ...
  )
{
  ...
  TitlePanel = GetPanelInfoByType (&mDEPrivate->Layout->PanelListHead, H2O_PANEL_TYPE_FORM_TITLE);
  if (TitlePanel == NULL) {
  ...
  if (TitleString != NULL) {
    ...
    BorderLineWidth = GetBorderWidth (TitlePanel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL);
    SetRect (
      &Control.ControlField,
      0,
      0,
  ...
  if (CoreVersionString != NULL) {
    ...
    StringWidth = (UINT32) (GetStringWidth ((CHAR16 *) CoreVersionString) / sizeof (CHAR16)) - 1;

    SetRect (
      &Control.ControlField,
      TitlePanel->PanelField.right - StringWidth,
      0,
  ...
  )
{
  ...
  if (SetupPagePanel != NULL) {
    SetSetupPageControlsField (SetupPagePanel);
    DisplaySetupPageControls (SetupPagePanel);
  }
  ...
  )
{
  ...
  BorderWidth        = GetBorderWidth (Panel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL);
  SeparationWidth    = 1;
  ...
  if ((Dialog->DialogType & H2O_FORM_BROWSER_D_TYPE_SHOW_CONFIRM_PAGE) == H2O_FORM_BROWSER_D_TYPE_SHOW_CONFIRM_PAGE) {
    Panel = GetPanelInfoByType (&mDEPrivate->Layout->PanelListHead, H2O_PANEL_TYPE_SETUP_PAGE);
    if (Panel != NULL) {
      UnionRect (&DialogField, &DialogField, &Panel->PanelField);
    }
  ...
  )
{
  ...
  IsHelpDialog = ((Dialog->DialogType & H2O_FORM_BROWSER_D_TYPE_SHOW_HELP) != 0 || (Dialog->DialogType & H2O_FORM_BROWSER_D_TYPE_SHOW_CONFIRM_PAGE) != 0) ? TRUE : FALSE;
  ...
  QuestionPanel->PanelRelField.bottom = QuestionPanel->PanelField.bottom - QuestionPanel->PanelField.top -
                                        GetBorderWidth(QuestionPanel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL) * 2;
  ...
  )
{
  ...
  if (Dialog->TitleString != NULL && QuestionPanel->ControlList.ControlArray != NULL) {
    Status = GetPanelColorAttribute (QuestionPanel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, PseudoClass, &ColorAttribute);
    if (EFI_ERROR (Status)) {
    ...
    TitleBorderLineField.top    = QuestionPanel->PanelField.top +
                                  GetBorderWidth (QuestionPanel, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL) +
                                  TitleStrHeight;
  ...
  )
{
  ...
  return DEConOutOutputString (mDEPrivate, String);
}
