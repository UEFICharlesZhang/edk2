  ...
  )
{
  ...
  if (NeedShowSetupMenu ()) {
    Status = GetLayoutById (PcdGet32 (PcdLayoutIdLmde), &LayoutInfo);
  } else if (IsFrontPage ()) {
    Status = GetLayoutById (PcdGet32 (PcdLayoutIdLmdeFrontPage), &LayoutInfo);
    DisableHalo (This);
  } else {
    Status = GetLayoutById (PcdGet32 (PcdLayoutIdLmdeCommonPage), &LayoutInfo);
  }
  ...
  ScreenField.left   = 0;
  ScreenField.top    = 0;
  ScreenField.right  = (INT32) PcdGet32 (PcdDisplayEngineLocalMetroHorizontalResolution);
  ScreenField.bottom = (INT32) PcdGet32 (PcdDisplayEngineLocalMetroVerticalResolution);
  ...
  while (PanelVfcfDataPtr->PanelName != NULL) {
    ...
    if (PanelInfo != NULL) {
      GetPanelField (PanelInfo, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL, &ScreenField, &PanelField);
      IsVisible = IsVisibility (PanelInfo, NULL, NULL, NULL, NULL, H2O_IFR_STYLE_TYPE_PANEL, H2O_STYLE_PSEUDO_CLASS_NORMAL);
    }
