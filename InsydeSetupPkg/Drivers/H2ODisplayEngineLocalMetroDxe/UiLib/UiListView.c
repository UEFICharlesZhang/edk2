  ...
  )
{
  ...
  if (Control->VScrollBar != NULL) {
    ...
    if (Direction == VK_PRIOR) { 
      ...
      if ((ChildRc.top >= ListRc.top) && (This->CurSel != 0)) {
      ...
      // Get control rectangle of current selection.
      Child = Control->Items[This->CurSel];
      Control->Manager->GetControlRect (Control->Manager, Child, &CurrentChildRc);
      ...
      if (SelectIndex == 0) {
        ...
        // Calculate the maximum dy.
        Child = Control->Items[0];
        Control->Manager->GetControlRect (Control->Manager, Child, &ChildRc);
        Max_dy = ChildRc.top - CurrentChildRc.top;
        ...
        for (Index = 0; Index < (INT32)Control->ItemCount; Index++) {
          ...
          if ((ChildRc.top - dy) >= ListRc.top) {
            ...
            if (dy <= Max_dy) {
              ...
              if (!(CONTROL_CLASS_GET_STATE (Child) & UISTATE_DISABLED) && (Index != This->CurSel)) {
                dy = dy - (ChildRc.top - CurrentChildRc.top);
              }
            }

            if (ChangeSelection == FALSE) {      
              for (Index2 = (This->CurSel - 1); Index2 != Index; Index2--) {
    ...
    if (Direction == VK_NEXT) { 
      ...
      if ((ChildRc.bottom <= ListRc.bottom) && (This->CurSel != (INT32)(Control->ItemCount - 1))) {
        for (Index = (This->CurSel + 1); Index <= (INT32)(Control->ItemCount - 1); Index++) {
          Child = Control->Items[Index];
          Control->Manager->GetControlRect (Control->Manager, Child, &ChildRc);  
          if (!(CONTROL_CLASS_GET_STATE (Child) & UISTATE_DISABLED)) {
            NotChange = FALSE;
        ...
        for (Index = ((INT32)Control->ItemCount - 1); Index >= 0; Index--) {
          ...
          if ((ChildRc.bottom - dy) < ListRc.bottom) {
            ...
            if (dy >= Max_dy) {
              ...
              if (!(CONTROL_CLASS_GET_STATE (Child) & UISTATE_DISABLED) && (Index != This->CurSel)) {
                dy = dy - (ChildRc.bottom - CurrentChildRc.bottom);
              }  
            }     
