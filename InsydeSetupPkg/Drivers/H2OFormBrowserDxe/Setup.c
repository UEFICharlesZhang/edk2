  ...
  )
{
  ...
  LoadAllFormSet         = HandleCount > 1 ? TRUE : FALSE;
  if (LoadAllFormSet) {
    LoadHiiFormset (&Handles[1], HandleCount - 1);
  ...
  // process handle list
  //
  Private->HiiHandleList  = AllocateCopyPool (HandleCount * sizeof (EFI_HII_HANDLE), Handles);
...

  {
    ...
    do {
      ...
      if (LoadAllFormSet && Selection->Action == UI_ACTION_REFRESH_FORMSET) {
      ...
      //
    if (!IsNvUpdateRequiredForFormSet (FormSet) && !LoadAllFormSet) {
  ...
  if (LoadAllFormSet) {
    DestroyLoadHiiFormset (Handles, HandleCount);
